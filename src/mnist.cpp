...
{
    std::string configFile_path = std::string(argv[1]);
    std::string path_testable_images, path_testable_label, path_trainable_label, path_trainable_images,log_file;
    double batch, epoch, learning_rate,hidden_size;
    Parser parser(configFile_path);
    parser.loader();
    std::map<std::string,std::string> data = parser.getting_data();
    path_testable_images = data["rel_path_test_images"];
    path_testable_label = data["rel_path_test_labels"];
    path_trainable_images = data["rel_path_train_images"];
    path_trainable_label = data["rel_path_train_labels"];
    log_file = data["rel_path_log_file"];
    stringToDouble(data["batch_size"],batch);
    stringToDouble(data["num_epochs"],epoch);
    stringToDouble(data["learning_rate"],learning_rate);
    stringToDouble(data["hidden_size"],hidden_size);
    //===================================================================================================
