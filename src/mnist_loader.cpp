...
void MNIST::read_dataset_labels(const std::string& file_path)
{
    ...
    for(int i = 0; i < static_cast<size_t>(numItems) ;++i)
    {
        ...
        //std::cout << "the "<<i<<"th of ";
        //std::cout<<static_cast<double>(label)<<"   "<<std::endl;
        //std::cout<<std::endl<<std::endl;
    }
...
const Eigen::MatrixXd& MNIST::getting_Images() const
{
   return images2D;
}
const Eigen::MatrixXd& MNIST::getting_Labels() const
{
    return labels2D;
}
const float& MNIST::getting_imageRows() const
{
    return imageRows;
}
const float& MNIST::getting_imageCols() const
{
    return imageCols;
}
