...
Neural_Network::Neural_Network(const int input_size, const int hidden_size, const int output_size)
    : input_size(input_size)
    , hidden_size(hidden_size)
    , output_size(output_size)
    , weightsLayer1(Eigen::MatrixXd::Zero(hidden_size, input_size + 1))
    , weightsLayer2(Eigen::MatrixXd::Zero(output_size, hidden_size + 1))
    , m1(Eigen::MatrixXd::Zero(hidden_size, input_size + 1))
    , v1(Eigen::MatrixXd::Zero(hidden_size, input_size + 1))
    , m2(Eigen::MatrixXd::Zero(output_size, hidden_size + 1))
    , v2(Eigen::MatrixXd::Zero(output_size, hidden_size + 1))
    , hidden_layer_output(Eigen::MatrixXd::Zero(0, 0))
    , output_layer_output(Eigen::MatrixXd::Zero(0, 0))
    , output_layer_soft_max(Eigen::MatrixXd::Zero(0, 0))
    , output_layer_loss(Eigen::MatrixXd::Zero(0, 0))
    , relu_input(Eigen::MatrixXd::Zero(0, 0))
    , hidden_layer_with_bias(Eigen::MatrixXd::Zero(0, 0))
    , input_with_bias(Eigen::MatrixXd::Zero(0, 0))
{
    // Initialize weights using Xavier initialization
    std::random_device rd;
    std::mt19937 gen(rd());
    
    const double limit1 = std::sqrt(6.0 / (input_size + hidden_size));
    std::uniform_real_distribution<> dis1(-limit1, limit1);
    for(int i = 0; i < weightsLayer1.rows(); ++i) {
        for(int j = 0; j < weightsLayer1.cols(); ++j) {
            weightsLayer1(i,j) = dis1(gen);
        }
    }
            
    const double limit2 = std::sqrt(6.0 / (hidden_size + output_size));
    std::uniform_real_distribution<> dis2(-limit2, limit2);
    for(int i = 0; i < weightsLayer2.rows(); ++i) {
        for(int j = 0; j < weightsLayer2.cols(); ++j) {
            weightsLayer2(i,j) = dis2(gen);
        }
    }
}
...
Neural_Network::~Neural_Network() {
    ...
    weightsLayer1.resize(0,0);
    weightsLayer2.resize(0,0);
    m1.resize(0,0);
    v1.resize(0,0);
    m2.resize(0,0);
    v2.resize(0,0);
    hidden_layer_output.resize(0,0);
    output_layer_output.resize(0,0);
    output_layer_soft_max.resize(0,0);
    output_layer_loss.resize(0,0);
    relu_input.resize(0,0);
    hidden_layer_with_bias.resize(0,0);
    input_with_bias.resize(0,0);
